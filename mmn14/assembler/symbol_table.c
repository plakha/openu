...
struct symbol
{
    ...
    dll *referred_addresses; /*where referred to. will keep it anyway! will print out only for extern 
    default: null (pro = less space) or empty dll (pro - less if checks)?*/

    size_t data_n_members;
    enum data_type data_type;

    dvec *num_data_vector; /* single STRING[80] or array of words or array of half words or array of bytes */
    char str[80];
};
